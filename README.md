...
as well as whether or not `dsn` is required as a parameter for `tbl2dframe` and `tbl2darray`.
...
`compute` and `db_save_view` allow you to save your `tbl` objects to tables and views in HP Vertica, respectively. Once saved as a view or table, they can be transported to Distributed R via `tbl2dframe` and `tbl2darray` (described in the following section). For a given `tbl` named `foo`:
...
## Using HP Distributed R's Native-Data-Loader with tbl2darray and tbl2dframe (requires vRODBC)
...
The functionality is available in the [HPdata][4] package, along with a standard ODBC connector interface. `HPdata` is another *soft* dependency of vertica.dplyr, meaning that vertica.dplyr only attempts to load it when either `tbl2darray` or `tbl2dframe` is invoked.

It is possible to convert a `tbl_vertica` object to a Distributed R `dframe` or `darray`. You must provide a `tbl_vertica` object to either `tbl2darray` or `tbl2dframe` if using a vRODBC-backed tbl_vertica, or the `tbl_vertica` object and a `dsn` reference for ODBC if using a RJDBC-backed tbl_vertica.
...
Additional parameters to `tbl2dframe` and `tbl2darray` exist, such as the number of partitions for the constructed dframe or darray. See the manual page for details.
...
## Supported dplyr-base functions
